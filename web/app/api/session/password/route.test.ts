...
/**
 * The security-critical half of password sign-in: credentials are exchanged
 * server-side and the tokens must reach the browser only as httpOnly cookies,
 * never in a response body a script could read.
 */
function post(body: unknown) {
  return POST(
    new Request('http://localhost:3000/api/session/password', {
      method: 'POST',
...
describe('POST /api/session/password', () => {
  beforeEach(() => {
    apiReturns({
  ...
  it('exchanges credentials at the API and stores both tokens in cookies', async () => {
    const res = await post(credentials);
    expect(res.status).toBe(200);

    const cookie = res.headers.get('set-cookie') ?? '';
    expect(cookie).toContain(`${TOKEN_COOKIE}=tok-123`);
    expect(cookie).toContain(`${REFRESH_COOKIE}=ref-456`);
    expect(cookie).toContain('HttpOnly');
    expect(cookie).toContain('SameSite=lax');
  });

  it('never returns either token in the response body', async () => {
