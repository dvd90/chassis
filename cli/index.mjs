...
/**
 * Paths the template itself declares as repository-only, via .chassisignore.
 *
 * This CLI is versioned separately from the template it downloads, so a
 * release from months ago still fetches today's master. Letting the template
 * declare its own exclusions means a new top-level directory does not leak
 * into projects generated by already-published versions.
 */
function declaredExclusions(src) {
  const declared = new Set(['.chassisignore']);
  const file = path.join(src, '.chassisignore');
...
function copyTemplate(src, dest) {
  const declared = declaredExclusions(src);

  fs.cpSync(src, dest, {
    recursive: true,
    filter: (from) => {
      if (IGNORE.has(path.basename(from))) return false;
      if (declared.has(path.basename(from))) return false;
      const rel = path.relative(src, from).split(path.sep).join('/');
      return !declared.has(rel);
    }
  });
}
